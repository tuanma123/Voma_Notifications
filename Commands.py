...
def get_reply_phone(message_received, phone_number):
    ...
    is_admin = Database.is_admin(phone_number)
    msg = ""
    if "paid" in command.lower():
        Database.update_rent(phone_number)
        msg = "Thank you for your payment"

    elif "announce" in command.lower() and is_member and Database.announcement_enabled(phone_number):
        msg = "Announcement by " + str(Database.get_name(phone_number)) + "\n" + \
